...
class Seq2SeqBase(SentencePlanner):
    ...
    def _beam_search(self, enc_inputs, da):
        ...
        for step in range(len(dec_inputs)):
            ...
            def cmp_func(p, q):
                ...
                return (int(p.logprob / (len(p) ** self.length_norm_weight) > q.logprob / (len(q) ** self.length_norm_weight)) - int(p.logprob / (len(p) ** self.length_norm_weight) < q.logprob / (len(q) ** self.length_norm_weight)))

            paths = sorted(new_paths, key=cmp_to_key(cmp_func), reverse=True)[:self.beam_size]
    ...
    def _rerank_paths(self, paths, da):
        ...
        return sorted(paths, key=cmp_to_key(lambda p, q: (int(p.logprob > q.logprob) - int(p.logprob < q.logprob))), reverse=True)
...
class Seq2SeqGen(Seq2SeqBase, TFModel):
        ...
        if self.multiple_refs:
            ...
            if refs_stored == 'serial':
                ...
                self.valid_trees = [[chunk[i] for chunk in valid_tree_chunks]
                                    for i in range(valid_size / num_refs)]
        ...
        if self.multiple_refs:
            ...
            if refs_stored == 'serial':
                ...
                self.valid_trees = [[chunk[i] for chunk in train_tree_chunks]
                                    for i in range(train_size / num_refs - self.validation_size,
                                                    train_size / num_refs)]
    ...
    def _init_neural_network(self):
        ...
        tf.set_random_seed(rnd.randint(-2147483648, 2147483647))
        ...
        if self.cell_type.startswith('gru'):
            ...
            self.cell = tf.nn.rnn_cell.LSTMCell(self.emb_size,name='basic_lstm_cell')
